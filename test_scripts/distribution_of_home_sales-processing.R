...
setkey(data, `Town/County`, `Year`)
setDT(backfill)
setDT(data)
setkey(backfill, `Town/County`, `Year`)
#data <- merge(data, backfill, all.y = T)
data <- data[backfill]
setkey(data, `Town/County`)
# library(dplyr)
# data2 <- right_join(data, backfill, by=c('Town/County' = 'Town/County', 'Price Range' = 'Price Range'))
...
data <- data[backfill] #needs to have Year in it
# cleanup
