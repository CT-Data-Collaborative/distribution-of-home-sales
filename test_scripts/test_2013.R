...
# For debugging purposes set year to 2013 and compare to existing town profiles
data_2013 <- raw[
  Year == 2013,
  list(Value = .N),
...
data_with_price_2013 <- merge(data_2013, backfill_price, all.y = T) #NAs present
...
data_with_price_and_towns_2013 <- merge.data.frame(data_with_price_2013, backfill_towns, all.y = T) #NAs present
...
# Reorder columns
setcolorder(data_all_2013, c("Town/County", "FIPS", "Year", "Price Range", "Measure Type", "Variable", "Value"))
...
data_all_2013 <- data_all_2013[order(`Town/County`, `Price Range`)]
#any(is.na(data_all_2013))
...
write.table(
  data_all,
  file.path(getwd(), "data", "distribution-of-home-sales_2013-2014.csv"),
  sep = ",",
  row.names = F,
  na = "-9999"
)
