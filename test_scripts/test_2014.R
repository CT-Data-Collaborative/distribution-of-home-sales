...
# For debugging purposes set year to 2014 and compare to existing town profiles
data_2014 <- raw[
  Year == 2014,
  list(Value = .N),
...
data_with_price_2014 <- merge(data_2014, backfill_price, all.y = T) #NAs present
...
data_with_price_and_towns_2014 <- merge.data.frame(data_with_price_2014, backfill_towns, all.y = T) #NAs present
...
# Reorder columns
setcolorder(data_all_2014, c("Town/County", "FIPS", "Year", "Price Range", "Measure Type", "Variable", "Value"))
...
data_all_2014 <- data_all_2014[order(`Town/County`, `Price Range`)]
#any(is.na(data_all_2014))
...
write.table(
  data_all,
  file.path(getwd(), "data", "distribution-of-home-sales_2014-2014.csv"),
  sep = ",",
  row.names = F,
  na = "-9999"
)
